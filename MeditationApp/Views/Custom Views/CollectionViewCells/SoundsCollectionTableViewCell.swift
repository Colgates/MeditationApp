//
//  SoundsCollectionTableViewCell.swift
//  MeditationApp
//
...
protocol SoundsCollectionTableViewCellDelegate: AnyObject {
    func didSelectItem(with model: Item)
}

class SoundsCollectionTableViewCell: UITableViewCell {

    public weak var delegate: SoundsCollectionTableViewCellDelegate?
    
    static let identifier = "SoundsCollectionTableViewCell"
    ...
    private func configureDataSource() {
        dataSource = UICollectionViewDiffableDataSource<Int, Item>(collectionView: collectionView, cellProvider: { collectionView, indexPath, model in
            ...
            cell.configure(with: model, titleIsHidden: true)
    ...
    func configure(with model: Sound) {
        configureDataSource()
...
extension SoundsCollectionTableViewCell: UICollectionViewDelegate {
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        
        delegate?.didSelectItem(with: (dataSource?.itemIdentifier(for: indexPath))!)
    }
}

// MARK: - CollectionView Item Size
extension SoundsCollectionTableViewCell: UICollectionViewDelegateFlowLayout {
